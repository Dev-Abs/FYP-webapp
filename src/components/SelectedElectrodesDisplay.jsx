...
const SelectedElectrodesDisplay = ({
  selectedElectrodes = [],
  small = false,
  enhancedDisplay = false,
}) => {
  const [activeRegion, setActiveRegion] = useState(null);
  const [showEnhancedDisplay, setShowEnhancedDisplay] =
    useState(enhancedDisplay);
  ...
  const toggleEhnancedDisplay = () => {
    setShowEnhancedDisplay(!showEnhancedDisplay);
  };
  ...
  const regionCounts = Object.keys(electrodeGroups).reduce((acc, region) => {
    ...
    const selected = regionElectrodes.filter((id) =>
      selectedElectrodes.includes(id)
    );
    acc[region] = {
  ...
  return (
    <div className="rounded-lg p-4">
      <div className="flex flex-col md:flex-row gap-4">
        ...
        <div
          className={`relative ${small ? "w-40 h-40" : "w-60 h-60"} mx-auto`}
        >
          ...
          <div className="absolute inset-0 rounded-full overflow-hidden">
            ...
            <div className="absolute inset-0">
              <SkullSVG activeGroup={activeRegion} onGroupClick={() => {}} />
            </div>
          </div>
          ...
          {selectedElectrodes.length > 1 && enhancedDisplay && (
            <svg className="absolute inset-0" viewBox="0 0 100 100">
              ...
              {selectedElectrodes.map((id, idx) => {
                ...
                const prevElectrode = allElectrodes.find(
                  (e) => e.id === selectedElectrodes[idx - 1]
                );
            ...
            <div className="text-center py-4">
              ...
              {enhancedDisplay && !small && (
                <p className="text-gray-500 text-xs mt-1">
                  Using default configuration (all electrodes)
                </p>
              )}
            </div>
          ) : (
              ...
              <div className={`mb-3 ${small ? "hidden" : ""}`}>
                ...
                <div className="h-2 bg-gray-700 rounded-full overflow-hidden">
                  <motion.div
                    ...
                    animate={{
                      width: `${
                        (selectedElectrodes.length / allElectrodes.length) * 100
                      }%`,
                    }}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="h-1.5 bg-gray-700 rounded-full overflow-hidden mt-1">
                        <motion.div
                          className={`h-full ${
                            enhancedDisplay && data.count > 0
                              ? "bg-gradient-to-r from-indigo-500 to-blue-500"
                              : ""
                          }`}
                          style={{
                            backgroundColor: enhancedDisplay
                              ? undefined
                              : data.color,
                            boxShadow:
                              enhancedDisplay && data.count > 0
                          ...
                          transition={{
                            ...
                            delay:
                              0.3 +
                              Object.keys(regionCounts).indexOf(region) * 0.1,
                          }}
                        />
                      </div>
                    </div>
      ...
      {/* Optional button to toggle enhanced display */}
      <div className="mt-[-20px] lg:mt-[-40px] lg:ml-3 lg:flex-row flex flex-row-reverse">
        <button
          ...
          onClick={() => toggleEhnancedDisplay()}
        >
          {showEnhancedDisplay
            ? "Disable Enhanced Display"
            : "Enable Enhanced Display"}
        </button>
      </div>
      {/* Visualization data display */}
      {showEnhancedDisplay && !small && selectedElectrodes.length > 0 && (
